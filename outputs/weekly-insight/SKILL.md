...
生成md文件后、生成JSON和DOCX之前，必须对报告进行四项自检。**任何一项不通过，就回到Step 3重新构思，从空白重写，不许在原稿上打补丁。**
...
- **子标题下的内容是否切题？** 每段内容必须直接服务于该子标题所宣称的论点。如果一个子标题叫"安全不是护城河，是城门"，其下的段落就必须在论证"安全=准入门槛"这个判断，而不是顺便塞入与安全无关的事件。发现跑题内容时，要么删除，要么移到它真正属于的子标题下，要么说明它为什么与当前论点相关。

**检查3：反向适配测试（反重复检查）**
每周的thesis必须通过反向适配测试：将本周论点套用在前四周（W-1到W-4）的事件上，如果也能完整成立，则视为重复，必须回到Step 3重新提炼。例如，如果W16的主题是"评估跟不上部署"，W17的论点也是"能力足够但可靠性不足"，则本质上是同一个判断的不同表述，视为重复。通过标准：本周论点必须依赖本周特有的事件组合才能成立，换成前几周的事件则论证链条会断裂。
...
**如果四项检查都通过**，继续生成JSON和DOCX。如果任何一项不通过，必须重写——回到Step 3，从原始数据重新构思论点和论证结构。

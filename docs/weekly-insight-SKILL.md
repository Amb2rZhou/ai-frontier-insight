...
Figure out the week to cover. Usually this is the previous Monday–Sunday (ISO week). Use the current date to calculate.
...
**Fact-check all verifiable claims via web search** before writing:
- Event dates and schedules (search for official pages)
- Funding amounts (confirm exact figures)
- Company metrics (employee counts, valuations, stock moves)
- Technical specifications (verify against official announcements)
- If a claim cannot be verified, label it as unverified in the report
...
All specific numbers, dates, and event timelines MUST be verified via web search. See Step 4 for details.
